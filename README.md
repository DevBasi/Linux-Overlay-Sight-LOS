...
Некоторые жанры (шутеры, MMO с видом от третьего лица) не показывают прицел в определённых ситуациях. Физические наклейки на монитор — неэстетично. AIM Overlay — программная замена: невидимое для системы окно с одним пикселем прицела ровно по центру экрана.
...
> _Добавьте сюда скриншот настроек и прицела в игре_

<details>
<summary>Стили прицела (ASCII-превью)</summary>

```
   dot          cross        dot+cross      circle
...
<img align="right" width="260" src="https://placehold.co/260x320/1a1a2e/00FF41?text=Settings+Dialog&font=mono" alt="settings">
...
### Почему обычные оверлеи не работают над fullscreen

Игры через Wine/Proton создают окно с флагом **`override-redirect`** — оно обходит KWin и рисуется прямо поверх всего. Стандартные трюки с `_NET_WM_STATE_ABOVE` не помогают, потому что KWin такие окна не видит.

### Решение

```
┌─────────────────────────────────────┐
│  X11 / XWayland stacking order      │
│                                     │
│  ┌─────────────────────────────┐    │
│  │  AIM Overlay                │ ← override-redirect + raise() каждые 250 мс
│  │  (X11BypassWindowManagerHint│    │
│  └─────────────────────────────┘    │
│  ┌─────────────────────────────┐    │
│  │  Игра (Wine / Proton)       │ ← тоже override-redirect (fullscreen)
│  └─────────────────────────────┘    │
│  ┌─────────────────────────────┐    │
│  │  Рабочий стол               │    │
│  └─────────────────────────────┘    │
...
Оверлей сам получает флаг `override-redirect` (`Qt::X11BypassWindowManagerHint`) и таймером каждые **250 мс** поднимается в верх X-стека — выше игрового окна. Клики при этом проходят насквозь через input shape, фокус не крадётся.
...
Физическая наклейка на монитор технически даёт тот же эффект — и никем не запрещается.
...
Сделано для Linux-геймеров, которым надоело клеить скотч на монитор
